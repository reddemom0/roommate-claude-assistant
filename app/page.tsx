        ...
        <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl p-6 w-full max-w-sm mx-4 border border-white/20">
          ...
          <div className="space-y-4">
            <input
              ...
              className="w-full px-5 py-4 border-0 bg-gray-50 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:bg-white outline-none text-lg transition-all duration-200"
              autoFocus
            />
            <button
              onClick={handleSetName}
              disabled={!userName.trim()}
              className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-4 rounded-2xl hover:from-indigo-700 hover:to-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all duration-200 font-medium text-lg shadow-lg transform active:scale-95"
            >
              Let's Go
            </button>
          </div>
    ...
    <div className="fixed inset-0 bg-gradient-to-br from-blue-50 to-indigo-100 flex flex-col" 
      ...
      <div className="bg-white/95 backdrop-blur-xl shadow-sm border-b border-white/20 flex-shrink-0">
        <div className="px-4 py-3 flex items-center justify-between">
          ...
          <button
            ...
            className="p-2 text-gray-500 hover:text-gray-700 transition-colors rounded-xl hover:bg-gray-100 active:scale-95 transform"
          >
        ...
        {/* Menu */}
        {showMenu && (
          <div className="border-t border-white/20 bg-white/80 backdrop-blur-xl p-4 space-y-3">
            <button
              onClick={clearChat}
              className="flex items-center space-x-2 text-gray-600 hover:text-gray-800 transition-colors w-full text-left p-2 rounded-xl hover:bg-gray-100 active:scale-95 transform"
            >
        ...
        <div className="flex space-x-2 overflow-x-auto pb-1 scrollbar-hide">
          {quickPrompts.map((prompt, index) => (
            <button
              ...
              className="flex-shrink-0 px-4 py-2 bg-gradient-to-r from-indigo-50 to-purple-50 text-indigo-600 rounded-full text-sm font-medium hover:from-indigo-100 hover:to-purple-100 transition-all duration-200 whitespace-nowrap border border-indigo-100 active:scale-95 transform"
            >
              {prompt}
            </button>
          ))}
      ...
      {pullDistance > 0 && (
        <div className="absolute top-20 left-1/2 transform -translate-x-1/2 z-10">
          <div className={`w-8 h-8 rounded-full bg-white shadow-lg flex items-center justify-center transition-transform duration-200 ${pullDistance > 60 ? 'bg-green-500' : 'bg-gray-200'}`}>
            <RotateCcw className={`w-4 h-4 ${pullDistance > 60 ? 'text-white' : 'text-gray-400'} transition-transform duration-200`} 
      ...
      <div className="flex-1 overflow-hidden">
        ...
        >
          {messages.map((message, index) => (
            ...
            >
              <div className={`max-w-[85%] px-4 py-3 rounded-3xl shadow-sm ${
                message.role === 'user'
      ...
      {/* Input Area */}
      <div className="bg-white/95 backdrop-blur-xl border-t border-white/20 p-4 flex-shrink-0">
        <div className="flex items-end space-x-3">
          <div className="flex-1">
            <textarea
              ...
              className="w-full px-4 py-3 border-0 bg-gray-50 rounded-3xl focus:ring-2 focus:ring-indigo-500 focus:bg-white outline-none resize-none text-base transition-all duration-200"
              rows={1}
              style={{
                minHeight: '48px',
                maxHeight: '120px',
          ...
          <button
            ...
            className="w-12 h-12 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-full hover:from-indigo-700 hover:to-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center flex-shrink-0 shadow-lg transform active:scale-95"
          >
